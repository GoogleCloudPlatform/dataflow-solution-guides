...
| Pub/Sub topic        |     `deadletter`     | The dead letter Pub/Sub topic for the sample pipeline.                                                                                                                                                                                                                                                |
| Pub/Sub subscription |    `input-sub`     | The subscription to the `input` topic that is actually used by the Dataflow pipeline.                                                                                                                                                                                                         |
| Pub/Sub subscription |   `deadletter-sub`   | The subscription to the `deadletter` topic, useful to temporarily store messages that pipeline cannot process due to errors.                                                                                                                                                                                          |
...
| Bigtable Instance    |     `clickstream-analytics`     | BigTable instance `clickstream-analytics` to store the information for enrichment of incoming messages                                                      |
| BigQuery Dataset       |     `clickstream_analytics`     | BigQuery dataset `clickstream_analytics`                                            |
| BigQuery Table       |     `wikipedia`     | BigQuery table `wikipedia` to store the processed records from dataflow job                                       |
