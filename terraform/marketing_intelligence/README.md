...
| :------------------- | :-------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Project              |                        Set by user                        | Optional, you may reuse an existing project. If the project is created by Terraform, it will enable the APIs for Cloud Build, Dataflow, Monitoring, Pub/Sub, Dataflow Autoscaling, Firestore, BigQuery, and Artifact Registry                                                                  |
...
| Firestore Database   |                         `(default)`                       | Cloud Firestore in Native Mode for serverless low-latency customer profile enrichment.                                                                                                                                                                                                            |
| BigQuery Dataset     |                     `output_dataset`                      | The output BigQuery dataset for storing scored customer interactions and propensity predictions.                                                                                                                                                                                                 |
...
| Service account      |                     `my-dataflow-sa`                      | Dataflow worker service account. It has storage admin, Dataflow worker, metrics writer, Pub/Sub editor, Datastore user, and BigQuery data editor roles assigned at project level.                                                                                                                |
...
| Firewall rules       |                       Several rules                       | Ingress and egress rules to remove unnecessary traffic, and to ensure the traffic required by Dataflow. If you want to access a VM using SSH, apply the network tag `ssh` to that instance. Same for `http-server` and for `https-server`                                                       |
...
| `destroy_all_resources` |  `bool`   | Optional. Default true. Destroy buckets and database resources with `tf destroy`.                                                                                                                                     |
| `network_prefix`         | `string`  | Optional. Default "dataflow". Add a prefix to the network net, subnet, NAT.                                                                                                                                           |
| `zone`                  | `string`  | Optional. Compute zone for resources. Default is "a".                                                                                                                                                                 |
