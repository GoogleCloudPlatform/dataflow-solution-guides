...
6. **Toolchain & Runtime Compatibility**: For Java pipelines, configure `build.gradle` with supported toolchains (`JavaLanguageVersion.of(25)`).
...
| 2 | **`cdp/`** *(Done)* | Pub/Sub Topics (`cdp-transactions`, `cdp-coupon-redemption`), BigQuery Dataset (`cdp_dataset`) & Table (`unified_customer_data`), Artifact Registry (`cdp-containers`) | `cdp-dataflow-sa` | `pipelines/cdp/` |
