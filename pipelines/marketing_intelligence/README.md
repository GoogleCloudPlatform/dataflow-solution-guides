...
- **Hermetic Custom Container**: Dependencies, pipeline code, and the serialized model (`marketing_model.pkl`) are pre-baked into a lightweight `python:3.14-slim` container, eliminating runtime PyPI downloads and startup lag.
...
├── Dockerfile                                     # Hermetic container definition (Python 3.14-slim + Beam 2.75.0)
...
├── marketing_model.pkl                            # Serialized Scikit-Learn propensity model artifact (generated via scripts/01_train_model.py)
