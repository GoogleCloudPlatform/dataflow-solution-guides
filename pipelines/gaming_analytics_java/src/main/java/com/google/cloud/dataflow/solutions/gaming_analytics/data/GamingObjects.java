...
public final class GamingObjects {
     ...
     * run with defaults. The type is deliberately narrowed to {@link ImmutableMap}: an enriched
     * event is a value flowing through a Beam pipeline, so nothing downstream may mutate it.
     */
    @DefaultSchema(AutoValueSchema.class)
    @AutoValue
    public abstract static class EnrichedEvent implements Serializable {
        ...
        public abstract ImmutableMap<String, String> getFeatures();
        ...
        public abstract static class Builder {
            ...
            /**
             * Takes a plain {@link Map} even though the property is an {@link ImmutableMap}, which
             * is what makes the narrowed getter safe to decode.
             *
             * <p>{@code AutoValueSchema} builds its decoder from this builder and hands the setter
             * its own {@code ByteBuddyUtils.TransformingMap}. A setter declared to take {@code
             * ImmutableMap} would make the generated bytecode fail verification with a {@code
             * VerifyError}, at runtime only. AutoValue generates {@code ImmutableMap.copyOf(value)}
             * for a widened setter, so the copy still happens exactly once and callers cannot
             * retain a handle on the stored map.
             */
