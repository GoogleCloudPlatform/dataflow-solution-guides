...
In the script `scripts/00_set_environment.sh` (automatically generated by running `terraform apply` in `terraform/iot_analytics`):

```bash
export PROJECT_ID=<YOUR_PROJECT_ID>
export PROJECT=<YOUR_PROJECT_ID>
export REGION=<YOUR_CLOUD_REGION>
export SUBNETWORK=<YOUR_SUBNETWORK_OR_EMPTY>
export NETWORK=$SUBNETWORK
export TEMP_LOCATION=gs://<YOUR_BUCKET>/tmp
export SERVICE_ACCOUNT=<YOUR_DATAFLOW_SERVICE_ACCOUNT>
...
export VEHICLE_DATA_PATH=<LOCAL_FILE_PATH_FOR_VEHICLE_DATA>
export MAINTENANCE_DATA_PATH=<LOCAL_FILE_PATH_FOR_MAINTENANCE_DATA>
export MODEL_FILE_PATH=<LOCAL_MODEL_FILE_PATH>

export MAX_DATAFLOW_WORKERS=3
```

Source the environment script to load all variables into your current shell:
...
Now, populate the Bigtable table with sample maintenance information:
...
To feed data into the pipeline, publish messages to the Pub/Sub topic `PUBSUB_TOPIC_ID` created via terraform:
