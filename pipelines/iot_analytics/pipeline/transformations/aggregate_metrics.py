...
class AggregateMetrics(beam.DoFn):
    ...
    VEHICLE_EVENTS_BAG = BagStateSpec('vehicle_events_bag', coders.registry.get_coder(VehicleStateEvent))
    MAX_TIMESTAMP = CombiningValueStateSpec('max_timestamp_seen', coders.IterableCoder(coders.TimestampCoder()),
                                           lambda elements: max(elements, default=Timestamp(0)))
    MAX_TEMPERATURE = CombiningValueStateSpec('max_temperature', coders.IterableCoder(coders.FloatCoder()), lambda elements: max(elements, default=0))
    MAX_VIBRATION = CombiningValueStateSpec('max_vibration', coders.IterableCoder(coders.FloatCoder()), lambda elements: max(elements, default=0))
    SUM_MILEAGE = CombiningValueStateSpec('sum_mileage', coders.IterableCoder(coders.FloatCoder()), lambda elements: sum(i for i in elements))
    COUNT_MILEAGE = CombiningValueStateSpec('count_mileage', coders.IterableCoder(coders.FloatCoder()),
    ...
    def window_expiry_callback(self,
        ...
        avg_mileage = sum_mileage.read() / count_mileage.read() if count_mileage.read() > 0 else 0.0
